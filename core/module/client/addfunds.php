...
if ($_SESSION["excel_userlogin"] != 1 || $user["client_type"] == 1) {
...
$pay_method_name = route(1);

if ($pay_method_name == 'payfastpaymentsuccess') {

    if (!$_SESSION['payfastpaymentsuccess']) {
        header('Location: /addfunds');
    }
    $_SESSION['payfastpaymentsuccess'] = false;
    $success = 1;
    $successText = 'Your Payment Was Successfull. Fund was Added to your Account';
}

if ($pay_method_name == 'payfastpaymentfailure') {

    if (!$_SESSION['payfastpaymentfailure']) {
        header('Location: /addfunds');
    }
    $_SESSION['payfastpaymentfailure'] = false;
    $error = 1;
    $errorText = 'Your Payment Was not Successfull. Try again or Contact Support';
}
if ($pay_method_name == 'payfast') {
    ...
    if ($_GET['err_code'] == '000' && ($_GET['err_msg'] == 'Success' || $_GET['err_msg'] == 'Processed%20OK')  && $_SESSION['basket_id'] == $_GET['basket_id']) {


        $tx_orderID = $_GET['transaction_id'];
        $order =  $conn->prepare('SELECT * FROM `payments` WHERE t_id=:tr_id and payment_status=:status');

        $order->execute(array('tr_id' => $tx_orderID, 'status' => 3));
        $order =   $order->rowCount();


        if ($order != 0) {

            $_SESSION['payfastpaymentfailure'] = true;
            header('Location: /addfunds/payfastpaymentfailure');
        } else {

            $baskid = $_GET['basket_id'];



            $select = $conn->prepare('select * from payments where client_id=:client_id and t_id=:t_id');

            $select->execute(array('client_id' => $user['client_id'], 't_id' => $baskid));
            $paymentData = $select->fetch(PDO::FETCH_ASSOC);

            $update = $conn->prepare('UPDATE `payments` SET payment_status=:payment_status, payment_delivery=:delivery where t_id=:t_id ');
            $check = $update->execute(array('payment_status' => 3, 't_id' => $baskid, 'delivery' => 2));

            $payment = $conn->prepare('SELECT balance FROM `clients` WHERE client_id=:client_id');
            ...
            $update2 = $update2->execute(array("id" => $paymentData["client_id"], "balance" => $payment["balance"] + $paymentData['payment_amount']));
            ...
            if ($update) {
                $success = 1;
                $easyj = 1;
                $successText = "Your Payment was Successfully Added";
                $_SESSION['basket_id'] = '';
                $_SESSION['payfastpaymentsuccess'] = true;
                header('Location: /addfunds/payfastpaymentsuccess');
            } else {
    ...
    if ($method->rowCount()) {
        ...
        if ($method["methodId"] == 18) {
         ...
         if ($method["methodId"] == 19) {
            require("addfunds/Initiators/payfast.php");
        }
        if ($method["methodId"] == 22) {
            require("addfunds/Initiators/bharatpe.php");
        }
        if ($method["methodId"] == 23) {
            require("addfunds/Initiators/nayapay.php");
        }
        if ($method["methodId"] == 24) {
            require("addfunds/Initiators/upaisa.php");
        }
        if ($method["methodId"] == 25) {
            require("addfunds/Initiators/zindigi.php");
        }
        if ($method["methodId"] == 26) {
            require("addfunds/Initiators/sadapay.php");
        }
        if ($method["methodId"] == 27) {
            require("addfunds/Initiators/jazzcashbusiness.php");
        }
        if ($method["methodId"] == 20) {
            require("addfunds/Initiators/uddoktapay.php");
        }
        if ($method["methodId"] == 69) {
            require("addfunds/Initiators/nagorikpay.php");
        }
        if ($method["methodId"] == 70) {
            require("addfunds/Initiators/piprapay.php");
        }
        header("Content-Type: application/json; charset=utf-8");
        echo json_encode($response, true);
        exit;
